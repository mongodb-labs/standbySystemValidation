...
func main() {
	...
	if *endpoint == "" || *bucket == "" {
		...
		fmt.Println("========================")
		fmt.Println()
		fmt.Println("WARNING: Do not use this binary on production buckets, prefer empty buckets instead.")
		fmt.Println()
...
func testPutObjectIfMatch(ctx context.Context, tc *TestContext) TestResult {
	...
	// Test wrong ETag. Use a valid MD5-formatted but non-matching ETag: strict S3
	// backends (e.g. Pure Storage FlashBlade) validate the ETag format and return
	// 400 BadRequest instead of 412 PreconditionFailed for malformed ETags.
